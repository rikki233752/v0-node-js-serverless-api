...
register(async ({ configuration, analytics, browser }) => {
  ...
  // Function to send event to our tracking API using sendBeacon
  function sendEventWithBeacon(eventName, eventData = {}, event) {
    try {
      ...
      // Use navigator.sendBeacon which is allowed in Shopify's sandbox
      if (navigator && navigator.sendBeacon) {
        ...
        if (success) {
          ...
          return { success: false, error: "sendBeacon failed" }
        }
      } else {
        // Fallback to image pixel tracking if sendBeacon is not available
        debugLog(`⚠️ [Web Pixel Gateway] sendBeacon not available, using image pixel fallback`)

        // Create a minimal query string with essential data
        const minimalData = `event=${encodeURIComponent(eventName)}&shop=${encodeURIComponent(shopDomain)}&t=${Date.now()}`

        // Create an image element to send the request
        const img = new Image(1, 1)
        img.src = `${TRACKING_API_URL}/pixel.gif?${minimalData}`

        debugLog(`✅ [Web Pixel Gateway] Sent ${eventName} event using image pixel fallback`)
        return { success: true, method: "image_pixel" }
      }
    } catch (error) {
