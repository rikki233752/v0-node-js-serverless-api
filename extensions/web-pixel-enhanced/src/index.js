...
register(async ({ configuration, analytics, browser }) => {
  ...
  function getShopDomain() {
    try {
      // Try to get from document.location first (most reliable)
      if (typeof document !== "undefined" && document.location) {
        const hostname = document.location.hostname
        if (hostname && hostname.includes("myshopify.com")) {
          debugLog(`✅ [Web Pixel Gateway] Detected shop domain from document.location: ${hostname}`)
          return hostname
        }
      }
      ...
      if (typeof document !== "undefined") {
        const metaShopId = document.querySelector('meta[name="shopify-shop-id"]')
        if (metaShopId) {
          debugLog(`⚠️ [Web Pixel Gateway] Found shop ID but no domain, using generic domain`)
          return "shopify-store.myshopify.com"
        }
      }
  ...
  function collectUserData() {
    ...
    try {
      ...
      if (typeof navigator !== "undefined" && navigator.userAgent) {
        ...
        debugLog(`✅ [Web Pixel Gateway] Added user agent to user data`)
      }
      ...
      if (typeof document !== "undefined" && document.cookie) {
        ...
        for (const cookie of cookies) {
          ...
          if (name === "_fbp") {
            userData.fbp = value
            debugLog(`✅ [Web Pixel Gateway] Added Facebook browser ID to user data`)
            break
          }
        }
      }
      ...
      if (!userData.em && !userData.ph && !userData.fbc && !userData.fbp) {
        ...
        const shopDomain = getShopDomain() || "unknown"
      ...
      return {
        ...
        client_user_agent: navigator?.userAgent || "Unknown",
      }
    }
  }
  ...
  async function sendEvent(eventName, eventData = {}) {
    try {
      ...
      const customData = {
        ...eventData,
        event_source_url: document?.location?.href || `https://${shopDomain}`,
  ...
  analytics.subscribe("page_viewed", async (event) => {
    try {
      // Extract shop domain from the event if possible
      let shopDomain = null
