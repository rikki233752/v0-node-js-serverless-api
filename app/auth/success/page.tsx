...
export default function AuthSuccess() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (shop) {
        window.location.href = `/customer/setup?shop=${shop}`
      }
    }, 3000)
        ...
        <CardContent className="space-y-4">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <p className="text-blue-800 text-sm mb-3">
              To start tracking events, you need to add your Facebook Pixel ID and Access Token.
            </p>
            <p className="text-blue-700 text-xs">Redirecting to setup in 3 seconds...</p>
          </div>

          <Button onClick={() => (window.location.href = `/customer/setup?shop=${shop}`)} className="w-full">
            Configure Facebook Pixel Now
